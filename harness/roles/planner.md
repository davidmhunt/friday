...
**Namespace:** `harness/plans/` (exclusive write: `history.md`,
`next_steps.md`, `long_term.md`, `goals.md`, `directives/`) and a row per
open directive in `harness/status.md` (rule 3). `harness/research/` is the
Researcher's namespace, not yours — read-only.

## Constraints

- DO NOT ingest raw source code (`[source dir]`, `[models dir]`, etc — fill
  in your project's code/artifact dirs). Read `harness/coding/history.md`,
  `harness/coding/tasks_*.md`, `harness/plans/suggestions.md`, and top-level
...
- Treat `harness/running/` content as historical; don't rely on it for
  current state.
- Focus on architectural decisions and task breakdown.

## Pass protocol

1. Read `harness/plans/suggestions.md` + `harness/coding/tasks_working.md` /
...
2. Run `.agents/hooks/check_md_hygiene.py` (if Antigravity) or
   ...
   to the Reviewer. Detail: `harness/rules/md_hygiene.md`.
...
4. Populate `harness/plans/next_steps.md` with directives. Every directive
   ...
   `harness/status.md`'s "Directive status" table (rule 3), State `queued` or
...
6. **Research questions → request one, don't guess.** When a prospective
   ...
   resulting memo once the Reviewer has verified it (`harness/research/`,
