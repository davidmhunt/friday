...
Run by the Reviewer per .friday/active/harness/roles/reviewer.md whenever a memo, theory
...
# "docs/research" is a consumer-repo-root path, same as the other two
# entries here, even though this tool itself is symlinked in from the
# .friday/ submodule (harness output for the harness's OWN content lives
# under .friday/active/harness/, but research memos are project content —
# see MANIFEST.json's dest_root: "repo" on that entry — so this scan
# reaches back out across the submodule boundary into the consumer repo).
SCAN_GLOBS = [
    ("docs/theory", "**/*.tex"),
    ("docs/report", "**/*.tex"),
    ("docs/research", "*.md"),
